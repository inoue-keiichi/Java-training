...
public class MainTest {
	...
	public void test() {
		...
		final Image actual = LatentImage.parallelFrom(image).parallelTransform(Color::brighter)
	...
	public void test_only_toImage() {
		final Image image = new Image(new File("./test/resource/image/sample.png").toURI().toString());
		final Image actual = LatentImage.parallelFrom(image).parallelToImage();
		final int width = (int) actual.getWidth();
		final int height = (int) actual.getHeight();
	...
	@Test
	public void test_executeParallel() throws Exception {
		throw new Exception("後で実装する");
	}
