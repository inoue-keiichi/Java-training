package dc3_4.frame.game.tetris;
...
import dc3_4.frame.game.tetris.clazz.Tetrimino;
import dc3_4.frame.game.tetris.clazz.TetrisField;
...
public class TetrisController implements Initializable, PropertyChangeListener {
	...
	public void initialize(final URL location, final ResourceBundle resources) {
		...
		//		this.es.execute(tetrisRunnable);
		this.es.execute(() -> {
			while (runFlag) {
				currentTetrimino.moveDown();
				update();
				try {
					Thread.sleep(currentTetrimino.getSpeed());
	...
	private void update() {
		...
		if (tetrisService.isOverlap(tetrisField.getColors(), currentTetrimino)) {
			...
			tetrisCanvas.getGraphicsContext2D().setFont(Font.font(25));
			tetrisCanvas.getGraphicsContext2D().fillText("GAME OVER", TetrisField.BLOCK_SIZE * 2,
	...
	private int calculateAdditionScore(int clearLineNum) {
		...
		return SCORE_BASIS_VALUE * level.get() * clearLineNum;
	}
	...
	private void addLevel() {
		...
		Platform.runLater(() -> {
			levelValue.setText(Integer.toString(level.get()));
		});
	...
	private void levelUpAnimation(final GraphicsContext gc) {
		gc.setFill(Color.WHITE);
		gc.fillText("LEVEL UP!", tetrisCanvas.getWidth() / 4, tetrisCanvas.getHeight() / 2);
		try {
			Thread.sleep(300);
		} catch (InterruptedException e) {
			// TODO 自動生成された catch ブロック
